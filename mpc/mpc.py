...
class MPC:
    ...
    def one_iter_mpc_control(self):
        ...
        if self.cbf_slack_activate:
            i = len(self.obstacle.obstacle_list)
            x1_opt = w_opt[0::6+i]
            x2_opt = w_opt[1::6+i]
            x3_opt = w_opt[2::6+i]
            x4_opt = w_opt[3::6+i]
        ...
        if self.show_evaluation:
            ...
            slack_exist = 'slack1_opt' in locals()
            if slack_exist:
                slacks = vertcat(DM(slack1_opt).T)
                #print(self.cost_function.eval_slacks_cost(slacks))
