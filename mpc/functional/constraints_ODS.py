    ...
    def __define_constraints_2trailers(self):
        ...
        for t in range(self.T):
            ...
            for i in range(len(self.obstacle.obstacle_list)):
                car_data_sym, _ = self.car.get_full_data_symbolic(self.x_sym[:, t])
                constraints += [sqrt((self.x_sym[0, t] - self.obstacle.obstacle_list[i]["center"][0])**2 +
                                     (self.x_sym[1, t] - self.obstacle.obstacle_list[i]["center"][1])**2) -
                                self.obstacle.obstacle_list[i]["radius"]]
                lower_boundary_constraints += [0]
                upper_boundary_constraints += [inf]
                constraints += [sqrt((car_data_sym['trailer1'][0] - self.obstacle.obstacle_list[i]["center"][0]) ** 2 +
                                     (car_data_sym['trailer1'][1] - self.obstacle.obstacle_list[i]["center"][1]) ** 2) -
                                self.obstacle.obstacle_list[i]["radius"]]
                lower_boundary_constraints += [0]
                upper_boundary_constraints += [inf]
                constraints += [sqrt((car_data_sym['tractor'][0] - self.obstacle.obstacle_list[i]["center"][0]) ** 2 +
                                     (car_data_sym['tractor'][1] - self.obstacle.obstacle_list[i]["center"][1]) ** 2) -
                                self.obstacle.obstacle_list[i]["radius"]]
                lower_boundary_constraints += [0]
                upper_boundary_constraints += [inf]
